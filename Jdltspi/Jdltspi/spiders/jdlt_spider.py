...
import scrapy
import time
from Jdltspi.settings import SEARCH_WORD,SEARCH
class Jdltspider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        try:
            url_open.find_element_by_xpath(".//td[@class='scbar_txt_td']/input[@id='scbar_txt']").send_keys(SEARCH_WORD)
        except:
            print('path错误')
        try:
        ...
        print(now_url)
        yield scrapy.Request(url=url_open.current_url,callback=self.parse)
        try:
            if url_open.find_element_by_partial_link_text('下一页'):
                url_open.find_element_by_partial_link_text('下一页').click()
